      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-10 md:mb-16 space-y-3 md:space-y-4">
          ...
          <p className="text-sm md:text-lg text-[#9D95B0] max-w-2xl mx-auto px-2 leading-relaxed">
            Гибкие тарифы под любой бюджет и цели. Все планы включают доступ к полной программе курса.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-8 max-w-md md:max-w-none mx-auto">
          {plans.map((plan, index) => (
            <div 
              key={index}
              className={`relative bg-[#16112A] border rounded-2xl md:rounded-3xl p-5 md:p-8 space-y-5 md:space-y-6 ${
                plan.popular 
            ...
            >
              ...
              <ul className="space-y-3 md:space-y-4">
                {plan.features.map((feature, featureIndex) => (
                  <li key={featureIndex} className="flex items-start gap-2 md:gap-3">
                    <svg className="w-4 h-4 md:w-5 md:h-5 text-purple-400 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 20 20">
                      <path d="M16.6667 5L7.5 14.1667L3.33333 10" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                    <span className="text-xs md:text-sm text-[#D1D0D8]">{feature}</span>
                  </li>
                ))}
              </ul>
              ...
              {/* CTA Button */}
              <button 
                className={`w-full py-3 md:py-3.5 rounded-full font-semibold transition-all text-sm md:text-base ${
                  plan.popular
